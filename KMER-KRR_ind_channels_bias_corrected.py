...
method = "KMER"
output_dir = "output_files_bias_corrected"
...
def kernel_mmd(X, gamma):
    ...
    for i in range(n_samples):
        for j in range(n_samples):
            ...
            diff_s = MMD_eeg_spectr_optimized(H_X, H_Z, nm=300)
            K[i, j] = np.exp(-gamma * diff_s*diff_s)
...
def accuracy_estimation(channel):
    ...
    for train_idx, test_idx in outer_cv.split(x, y, groups):
        ...
        train_age = y_train
        test_age = y_test
        train_bag = y_train - custom_predict(ridge_model, K_train)

        # Fit the bias correction model
        glm = LinearRegression()
        glm.fit(train_age.reshape(-1, 1), train_bag)

        # Apply bias correction to predictions
        correction = glm.predict(test_age.reshape(-1, 1))
        corrected_predictions = y_pred + correction

        fold_score = r2_score(y_test, corrected_predictions)
        fold_scores.append(fold_score)
        combined_outputs.extend(corrected_predictions)

        mae_fold = mean_absolute_error(y_test, corrected_predictions)
    ...
    if method == "KRR":
        plt.scatter(combined_y_test, np.array(combined_outputs), color="pink", alpha=0.75)
